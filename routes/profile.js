...
router.post('/bio', authenticateToken, async (req, res) => {
  try {
    ...
    await db4.query(
      `INSERT INTO profiles(user_id, bio, updated_at) VALUES($1,$2,NOW())
       ON CONFLICT (user_id) DO UPDATE SET bio=$2, updated_at=NOW()`,
      [userId, cleanBio] // FIXED
    );
    res.json({ success: true, bio: cleanBio });
  } catch (err) {
    console.error('[Bio Save Error]', err.message);
...
router.delete('/bio', authenticateToken, async (req, res) => {
  try {
    await db4.query(`UPDATE profiles SET bio='', updated_at=NOW() WHERE user_id=$1`, [req.user.userId]);
  ...
  try {
    ...
    await db4.query(
      `INSERT INTO profiles(user_id, avatar_file_id, avatar_bot_id, avatar_updated_at)
       VALUES($1,$2,$3,NOW()) ON CONFLICT (user_id) DO UPDATE SET avatar_file_id=$2, avatar_bot_id=$3, avatar_updated_at=NOW()`,
...
router.post('/face-verify', authenticateToken, async (req, res) => {
  try {
    ...
    await db4.query(`INSERT INTO profiles(user_id, face_hash, face_verified, updated_at) VALUES($1,$2,true,NOW()) ON CONFLICT (user_id) DO UPDATE SET face_hash=$2, face_verified=true, updated_at=NOW()`, [userId, faceHash]);
    try {
...
router.post('/id-upload', authenticateToken, async (req, res) => {
  try {
    ...
    await db4.query(
      `INSERT INTO profiles(user_id, id_photo_temp, id_status, updated_at) VALUES($1,$2,'PENDING',NOW())
       ON CONFLICT (user_id) DO UPDATE SET id_photo_temp=$2, id_status='PENDING', updated_at=NOW()`,
      [userId, idPhotoBase64] // FIXED
    );
